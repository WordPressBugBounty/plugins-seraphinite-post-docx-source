			...
			if( !$rawExtData )
			{
				if( $verFrom && Plugin::IsInited() )
				{
					...
					for( $verFromDel = ( int )$verFrom - 5; $verFromDel; $verFromDel-- )
						delete_option( 'seraph_pds_' . $name . 'V' . $verFromDel );
				}
	...
	static function Get( $postId, $ver, $name, $cbType = 'Post', $cbNs = '' )
	{
		...
		if( $ver != $verFrom )
		{
			if( $verFrom )
			{
				...
				for( $verFromDel = ( int )$verFrom - 5; $verFromDel; $verFromDel-- )
					delete_post_meta( $postId, '_seraph_pds_' . $name . 'V' . $verFromDel );
			}
...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.16.14' || $lastCheckPackage !== 'Base' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.16.14' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.16.14' )
				return( Gen::S_FALSE );
...
class Plugin
{
	...
	static function GetPluginString( $id )
	{
		...
		esc_html_x( 'PluginListSep', 'admin.Common', 'seraphinite-post-docx-source' );
		esc_html_x( 'PluginListTokenSep', 'admin.Common', 'seraphinite-post-docx-source' );
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.16.14' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.16.14' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.16.14' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static function Sett_SaveBtn( $name, $full = true )
	{
		...
		$res .=
			...
			Ui::Button( esc_html( Wp::GetLocString( 'Export' ) ), false, null, null, 'button', array( 'class' => 'ctlSpaceAfter ctlSpaceVAfter', 'style' => array( 'min-width' => '7em' ), 'onclick' => 'seraph_pds.PluginAdmin.SettExport(this,"' . $urlAct . '","' . wp_create_nonce( 'export' ) . '")' ) ) .
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.16.14' );
			if( $verTo < $verFrom )
...
?>
	...
	static private function _Eula_Check()
	{
		...
		$res .= '<input type="button" class="button button-primary" value="' . $acceptBtnName . '" style="margin-right: 0.5em;" onclick="window.location.href=\'' . add_query_arg( array( 'action' => 'seraph_pds_act', 'fn' => 'acceptEula', 'redir' => rawurlencode( esc_url( $_SERVER[ 'REQUEST_URI' ] ) ), '_' => time()  ), get_admin_url( NULL, 'admin-ajax.php' ) ) . '\'" />';
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.16.14' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.16.14' );
			if( $id == 'Gen' )
				Plugin::Loc_ScriptLoad( $scrHndId );
			wp_enqueue_script( $scrHndId );

			if( $id == 'Gen' )
				Wp::AddInlineScript( $scrHndId, 'seraph_pds.Plugin._int.urlRoot="' . wp_slash( Plugin::FileUri( '', __DIR__ ) ) . '";seraph_pds.Plugin._int.urlAdminApi="' . wp_slash( Plugin::GetAdminApiUri() ) . '";seraph_pds.Plugin._int.urlApi="' . wp_slash( Plugin::GetApiUri() ) . '";seraph_pds.Wp.Loc._int.lang="' . Lang::GetLangFromLocale( Wp::GetLocale() ) . '";' );
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.16.14" ) );
